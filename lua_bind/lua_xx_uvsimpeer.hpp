...
inline void Lua_Register_UvSimPeer(lua_State* const& L)
{
	using Peer = xx::UvSerialBBufferSimulatePeer;
	using Peer_s = std::shared_ptr<Peer>;
	Lua_NewMT(L, TypeNames<Peer_s*>::value, TypeNames<xx::UvItem_s*>::value);					// xx.UvSimPeer ： UvItem
	...
	{
		auto&& t = Lua_ToTuple<Peer_s*>(L, "GetId error! need 1 args: self");
		assert((*std::get<0>(t)));
		return Lua_Pushs(L, (*std::get<0>(t))->id);
	});


	Lua_NewFunc(L, "SendPush", [](lua_State* L)
	{
		auto&& t = Lua_ToTuple<Peer_s*, xx::BBuffer*>(L, "SendPush error! need 2 args: self, BBuffer");
		assert((*std::get<0>(t)));
	...
	{
		auto&& t = Lua_ToTuple<Peer_s*, int, xx::BBuffer*>(L, "SendResponse error! need 3 args: self, serial, BBuffer");
		assert((*std::get<0>(t)));
	...
	{
		auto&& t = Lua_ToTuple<Peer_s*, xx::BBuffer*, Lua_Func>(L, "SendRequest error! need 3 ~ 4 args: self, BBuffer, callback, timeoutMS = 0");
		assert((*std::get<0>(t)));
		int64_t timeoutMS = 0;
		if (lua_gettop(L) > 3)
		{
	...
	{
		auto&& t = Lua_ToTuple<Peer_s*, Lua_Func>(L, "OnDisconnect error! need 2 args: self, func/null");
		if (std::get<1>(t))
		{
	...
	{
		auto&& t = Lua_ToTuple<Peer_s*, Lua_Func>(L, "OnReceivePush error! need 2 args: self, func(bb)/null");
		if (std::get<1>(t))
		{
	...
	{
		auto&& t = Lua_ToTuple<Peer_s*, Lua_Func>(L, "OnReceiveRequest error! need 2 args: self, func(serial, bb)/null");
		if (std::get<1>(t))
		{
