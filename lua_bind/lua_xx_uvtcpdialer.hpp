...
{
	Lua_NewMT(L, TypeNames<xx::UvTcpLuaDialer_s*>::value, TypeNames<xx::UvItem_s*>::value);					// xx.UvTcpLuaDialer ： UvItem
	...
	Lua_NewFunc(L, "OnAccept", [](lua_State* L)
	{
		...
		if (std::get<1>(t))
		{
			(*std::get<0>(t))->OnAccept([f = std::move(std::get<1>(t))](xx::IUvPeer_s peer_)
			{
				if (!gLua) return;
				assert(!lua_gettop(gLua));
				auto&& L = gLua;

				auto&& peer = xx::As<xx::UvTcpLuaPeer>(peer_);
				Lua_PCall(L, f, peer);
				lua_settop(L, 0);
			});
		}
