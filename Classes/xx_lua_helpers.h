...
namespace xx
{
	...
	// 从注册表取出 mp
	inline MemPool* LuaGetMemPool(lua_State* L)
	{
		lua_pushlightuserdata(L, (void*)LuaKey_MemPool);
		lua_rawget(L, LUA_REGISTRYINDEX);
		auto r = (MemPool*)lua_touserdata(L, -1);
		assert(r);
		lua_pop(L, 1);
		return r;
	}
	...
	// 从注册表取出 uvloop
	inline UvLoop* LuaGetUvLoop(lua_State* L)
	{
		lua_pushlightuserdata(L, (void*)LuaKey_UvLoop);
		lua_rawget(L, LUA_REGISTRYINDEX);
		auto r = (UvLoop*)lua_touserdata(L, -1);
		assert(r);
		lua_pop(L, 1);
		return r;
	}
	...
	// 压 内存池 到 注册表
	inline void LuaRegisterMemPool(lua_State* L, MemPool* mp)
	{
		lua_pushlightuserdata(L, (void*)LuaKey_MemPool);
		lua_pushlightuserdata(L, mp);
		lua_rawset(L, LUA_REGISTRYINDEX);
		assert(LuaGetMemPool(L));
	}

	// 压 uvloop 到 注册表
	inline void LuaRegisterUvLoop(lua_State* L, UvLoop* uvloop)
	{
		lua_pushlightuserdata(L, (void*)LuaKey_UvLoop);
		lua_pushlightuserdata(L, uvloop);
		lua_rawset(L, LUA_REGISTRYINDEX);
		assert(LuaGetUvLoop(L));
	}
...
function MakeRef( o )
	...
	inline void LuaCreateFuncStore(lua_State* L, void* key)
	{
		lua_pushlightuserdata(L, key);				// key
		lua_createtable(L, 128, 0);					// key, cbfuncs
		lua_rawset(L, LUA_REGISTRYINDEX);			// 
	}

	// 将 lua cb func 放入位于注册表中的容器
	inline void LuaStoreFunc(lua_State* L, void* key, int funcId, int funcIndex)
	{
		lua_pushlightuserdata(L, key);				// ..., key
	...
	inline void LuaPushFunc(lua_State* L, void* key, int funcId)
	{
		lua_pushlightuserdata(L, key);				// ..., key
		lua_rawget(L, LUA_REGISTRYINDEX);			// ..., t
		lua_pushinteger(L, funcId);					// ..., t, funcId
	...
	// 从注册表移除函数
	inline void LuaRemoveFunc(lua_State* L, void* key, int funcId)
	{
		lua_pushlightuserdata(L, key);				// ..., key
		lua_rawget(L, LUA_REGISTRYINDEX);			// ..., t
		lua_pushinteger(L, funcId);					// ..., t, funcId
