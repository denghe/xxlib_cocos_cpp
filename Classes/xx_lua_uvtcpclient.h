...
namespace xx
{
	struct LuaUvTcpClient : public UvTcpClient
	{
		...
		inline static void LuaRegister(lua_State *L)
		{
			...
			LuaCreateFuncStore(L, (void*)name);
		}
		...
		inline static int Create_(lua_State* L)
		{
			...
			self->OnReceivePackage = [self](BBuffer& bb)
			{
				decltype(auto) m = self->messages.Emplace();
				m.MPCreate(self->mempool);
				m->typeId = 0;
			...
			self->OnReceiveRequest = [self](uint32_t serial, BBuffer& bb)
			{
				decltype(auto) m = self->messages.Emplace();
				m.MPCreate(self->mempool);
				m->typeId = 1;
		...
		inline static int SendRequest_(lua_State* L)
		{
			...
			LuaStoreFunc(L, (void*)name, funcId, 3);

			uint32_t serial = self.SendRequest(*(BBuffer*)bb, [self = &self, funcId](uint32_t ser, BBuffer* bb)
			{
				if (!self) return;
		...
		inline static int SendResponse_(lua_State* L)
		{
			...
			int r = self.SendResponse(serial, *(BBuffer*)bb);
			lua_pushinteger(L, r);
			return 1;
		}
		...
		inline int OnXxxx(lua_State* L, int& tarHandlerId)
		{
			...
			if (tarHandlerId != -1)
			{
				LuaRemoveFunc(L, (void*)name, tarHandlerId);
			}
			...
			LuaStoreFunc(L, (void*)name, tarHandlerId, 2);
		...
		inline static void PushBBuffer(lua_State* L, BBuffer& bb)
		{
			// 压 数据参数
		...
		inline static int PopMessage_(lua_State* L)
		{
			...
			{
			case -3:						// 连接断开
				LuaPushFunc(L, (void*)name, self.onDisconnectFuncId);
				return 2;
			case -2:						// 连接失败
				LuaPushFunc(L, (void*)name, self.onConnectFuncId);
				return 2;
			case -1:						// 请求回调超时
				LuaPushFunc(L, (void*)name, m->funcId);
				return 2;
			case 0:							// 收到普通包
				LuaPushFunc(L, (void*)name, self.onReceiveFuncId);
				PushBBuffer(L, m->bb);
				return 3;
			case 1:							// 收到请求包
				LuaPushFunc(L, (void*)name, self.onReceiveRequestFuncId);
				PushBBuffer(L, m->bb);
