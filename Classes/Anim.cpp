...
void AnimExt_CPP::SetPathway(size_t const& idx) {
...
int AnimExt_CPP::Update(float const& elapsedSeconds_) {
	if (timeScale == 0 || speedScale == 0) return 0;
	auto elapsedSeconds = elapsedSeconds_ * timeScale;

	// Update childAnims
	for (int i = (int)childAnims.size() - 1; i >= 0; --i) {
		auto& c = childAnims[i];
		if (c.first->Update(elapsedSeconds)) {
			c = std::move(childAnims[childAnims.size() - 1]);
			childAnims.pop_back();
		}
	}
	...
	const auto&& Move = [&](float const& d) {
		...
		bool r = false;
		if (speedScale > 0) {
			r = pathway->Forward(d * speedScale, pathwayI, pathwayD, anim->pos, anim->angle);
		}
		else {
			r = pathway->Backward(d * -speedScale, pathwayI, pathwayD, anim->pos, anim->angle);
		}
		// 同步子动画的位置和角度
		for (auto&& c : childAnims) {
			c.first->pos = anim->pos + xx::Rotate(c.second, anim->angle);
			c.first->angle = anim->angle;
		}
		return r;
	};
...
void AnimExt_CPP::Event(int const& eventId, float const& elapsedSeconds) {
...
bool AnimExt_CPP::IsIntersect(float const& x, float const& y, float const& r) const {
...
bool AnimExt_CPP::Lockable() const {
	if (action->lps.empty()) return false;
	// todo: 判断所有点都在可锁定区域/显示区域内
	return true;
}

xx::Point AnimExt_CPP::GetLockPoint() const {
	xx::Point rtv;
...
bool AnimExt_CPP::Attackable() const { return true; }
void AnimExt_CPP::Lock() {}
void AnimExt_CPP::Unlock() {}
void AnimExt_CPP::Attack() {}
void AnimExt_CPP::Death() {}

void AnimExt_CPP::Draw() {
	// Draw childs. 先子后父，输出画面为 父遮子
	for (auto&& c : childAnims) {
		c.first->Draw();
	}
...
#endif // DEBUG_DRAW_ANIM_INFO
}

AnimExt_CPP::~AnimExt_CPP() {
#ifdef DEBUG_DRAW_PATHWAY
	if (debug_path) {
